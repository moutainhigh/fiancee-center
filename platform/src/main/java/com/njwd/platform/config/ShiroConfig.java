...
public class ShiroConfig {
	...
	public RedisSessionDAO redisSessionDAO(RedisManager redisManager) {
		...
		RedisManager newRedisManager = new RedisManager();
		FastUtils.copyProperties(redisManager, newRedisManager);
		newRedisManager.setDatabase(PlatformConstant.ShiroConfig.DATABASE);
		redisSessionDAO.setRedisManager(newRedisManager);
		redisSessionDAO.setKeyPrefix(PlatformConstant.ShiroConfig.SESSION_KEY_PREFIX);
		redisSessionDAO.setExpire(PlatformConstant.ShiroConfig.EXPIRE);
	...
	public SessionsSecurityManager securityManager(UserRealm sysUserRealm, RedisManager redisManager, RedisSessionDAO redisSessionDAO) {
		...
		cacheManager.setKeyPrefix(PlatformConstant.ShiroConfig.CACHE_KEY_PREFIX);
	...
	private static class HeaderWebSessionManager extends DefaultWebSessionManager {
		...
		protected Serializable getSessionId(ServletRequest request, ServletResponse response) {
			String token = WebUtils.toHttp(request).getHeader(PlatformConstant.ShiroConfig.SESSION_ID_KEY);
			// 如果请求头中有值则从请求头中获取
